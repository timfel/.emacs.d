...
(declare-function treesit-simple-indent-add-rules "treesit")
...
(defun timfel/move-line-up (n)
  ...
  (interactive "p")
  (timfel/move-line (if (null n) -1 (- n))))
...
(defun timfel/java-indentation-setup ()
  (interactive)
  (set-fill-column 99)
  ...
  (when (derived-mode-p 'java-ts-mode)
    (treesit-simple-indent-add-rules
     'java
     '(((parent-is "formal_parameters") parent-bol 16)
       ((parent-is "argument_list") parent-bol 16))))
...
(defun noto-install-fonts ()
  (interactive)
  (when (yes-or-no-p "This will download and install fonts, are you sure you want to do this?")
    (let* ((font-dest (cond
                       ...
                       ((eq system-type 'android)
                        (file-name-concat user-emacs-directory "fonts/"))
      ...
      (let ((fonts '(("NotoColorEmoji.ttf"
                      . "https://raw.githubusercontent.com/googlefonts/noto-emoji/main/fonts/NotoColorEmoji-emojicompat.ttf")
                     ("NotoSansSymbols2-Regular.ttf"
                      . "https://raw.githubusercontent.com/google/fonts/main/ofl/notosanssymbols2/NotoSansSymbols2-Regular.ttf"))))
        (dolist (spec fonts)
          (let ((dest (expand-file-name (car spec) font-dest)))
            (unless (file-exists-p dest)
              (url-copy-file (cdr spec) dest t)))))
      (when known-dest?
        (message "Fonts downloaded, updating font cache... <fc-cache -f -v> ")
        (shell-command-to-string (format "fc-cache -f -v"))
        (shell-command-to-string (format "flatpak-spawn --host fc-cache -f -v")))
      (message "Successfully %s `noto' fonts to `%s'!"
...
(setq locate-dominating-stop-dir-regexp
      (concat locate-dominating-stop-dir-regexp
              (format "\\|\\`%s" (regexp-quote timfel/cloud-storage))))
